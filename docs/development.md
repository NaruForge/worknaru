...
사용자가 선택한 A 배색으로 Main Color 미리보기·적용·취소, 저장 실패와 다른 탭 반영을 체험한다. Light/Dark, 화면·상태·너비도 바꿀 수 있다. v3에서 공통 컨트롤과 설정의 Main 배치, 입력창 바로 위 파일 승인 위치를 선택했다. 사용자는 v5의 설정 전용 메뉴와 모바일의 Module → 목록 → Main 이동도 승인했다. `?width=390`으로 열면 모바일 너비에서 시작하며, 큰 목록 복귀 버튼과 브라우저 뒤로·앞으로 가기, 초안·목록 위치 유지를 체험한다. 제품은 승인된 탐색 방식을 적용한다. 사용자는 입력창 바로 위 배치를 선택했으며, 제품도 같은 공통 승인 패널을 사용한다. 사용자는 이슈 #25와 연결된 대화에 확인과 수정 의견을 남긴다. 최초 A/B 시안은 commit `1e31892`에 보존되어 있다.
...
Chat 입력창 바로 위의 **파일 수정 승인 대기** 패널을 펼쳐 경로와 파일 전체의 수정 전후 내용을 확인한다. **이번 수정 허용**은 해당 수정안만 적용하고, **거절**은 파일을 유지한 채 거절 결과를 AI에 전달한다. **실행 중지**는 대기 요청을 취소하고 해당 AI 실행을 정리한다. 패널을 접는 것은 승인·거절·중지가 아니며 같은 패널을 다시 펼칠 수 있다. 다른 대화의 요청에는 대화 제목을 함께 표시한다. 같은 Daemon에 재접속하면 저장된 대기 요청을 다시 표시한다. 승인 대기는 최대 5분이고, AI의 120초 응답 제한에는 이 대기 시간을 포함하지 않는다.
...
좌측 서비스 탐색 하단의 **설정(Settings)** 에서 Daemon 연결 설정, Codex 인증 상태, 새 대화의 기본 Model·Reasoning Effort를 확인한다. 설정은 화면·연결·AI 전용 메뉴를 가진 Main의 독립된 플랫폼 화면이다. Chat의 대화 목록을 재사용하지 않는다. 860px 미만에서는 Module 선택 → 설정 메뉴 → 해당 설정 화면으로 이동하며 큰 목록 복귀 버튼과 브라우저 뒤로·앞으로 가기를 사용한다. Chat으로 돌아오면 선택한 대화·초안·목록 위치와 본문의 읽던 위치를 유지하며, 넓은 화면의 목록 접힘 상태는 Chat과 설정이 따로 유지한다. 기존 상단 연결 상태 버튼도 연결 창을 연다. Permission에는 **Workspace 텍스트 파일 수정 · 요청마다 승인**을 표시한다. 로그인·로그아웃·계정 전환, 상시 허용과 범용 도구 권한 정책 편집은 제공하지 않는다.
...
설정 변경 전에 같은 Daemon에 `ai.get`을 호출해 제공자 목록을 확인한다. 미확인 목록은 `AI_CATALOG_REQUIRED`, 지원 밖 조합은 `MODEL_UNSUPPORTED`로 거절한다. 변경은 기존 요청 ID·epoch·접수 장부 규칙을 그대로 사용한다. 설정 저장과 접수 결과를 함께 커밋하며, 응답 유실 후 `requests.get`으로 확인한다. 최초 접수 스냅샷을 현재 설정으로 간주하지 않고 `settings.get` 또는 `sessions.get`을 다시 조회한다. 브라우저는 미확정 설정 요청도 같은 탭의 접수 장부에 보관하고 자동 재전송하지 않는다. 설정 변경은 작성 중인 메시지 초안을 지우지 않는다. 브라우저 탐색 기록은 Workspace·저장 세대를 구분하고 화면 선택과 Session 식별자만 담는다. 새로고침 뒤 같은 Workspace에 연결하면 현재 탐색 항목을 복원하지만, 보내지 않은 초안은 기존처럼 새로고침을 넘겨 저장하지 않는다. 다른 Workspace나 저장 세대에서는 대화 탐색을 초기화한다. 새 Workspace의 연결 초기화가 끝나기 전에도 브라우저 뒤로·앞으로 가기로 이전 Workspace의 대화를 조회하지 않는다. 본문 스크롤 위치와 최근 내용 따라가기는 현재 대화에 한해 메모리에서 유지하고, 대화·Workspace·저장 세대가 바뀌면 초기화한다. 화면 배색 Main에서 860px 경계를 오가도 입력 중인 HEX와 미리보기를 유지하며, 실제 설정 이탈이나 취소는 마지막 적용색으로 복귀한다.
