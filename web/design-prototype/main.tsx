...
function SceneView() {
  ...
  useLayoutEffect(() => {
    if (!wide) document.querySelector<HTMLElement>(view === 'modules' ? '.module-picker h1' : view === 'list' ? '.stacked-list [aria-current], .stacked-list .ui-button' : '.stacked-toolbar .ui-button')?.focus();
  }, [view, scene, wide]);
  ...
  return <><WorkspaceShell name="제품팀 Workspace" online={scenario !== 'error'} theme={mode} toggleTheme={() => { const p = new URLSearchParams(params); p.set('mode', mode === 'light' ? 'dark' : 'light'); setParams(p); }} connection={() => navigate({scene:'connection', view:'main'})} settings={() => navigate({scene:'settings', view:'list'})} chat={() => navigate({scene:'chat', view:'list'})} settingsActive={scene === 'settings'} sectionLabel={scene === 'settings' ? '플랫폼 설정' : undefined} sidebar={wide && expanded && !noList ? currentList : undefined} sidebarLabel={scene === 'settings' ? '설정 메뉴' : '대화 목록'} listOpen={expanded} toggleList={noList || !wide ? undefined : () => setExpanded(!expanded)} navigationLabel={scene === 'settings' ? '설정 메뉴 열기 또는 접기' : '대화 목록 열기 또는 접기'} stacked={!wide ? {view, list:hasList ? currentList : undefined, backLabel:view === 'main' && hasList ? scene === 'settings' ? '← 설정 메뉴' : '← 대화 목록' : '← Module', back} : undefined}>
    {mainPanel ? (wide || view === 'main') && <MainPanel title={scene === 'settings' ? '설정' : '공통 컨트롤'} back={wide ? close : undefined}>{scene === 'controls' ? <ControlSamples state={scenario} /> : <div className="settings-content">{settingsTab === 'appearance' && <AppearanceSettings preference={preference} />}{settingsTab === 'connection' && <section><h2>Workspace 연결</h2><p className="muted small">연결됨 · 예제 Workspace</p><Button variant="outline" onClick={() => navigate({scene:'connection', view:'main'})}>연결 설정 변경</Button></section>}{settingsTab === 'ai' && <><section><h2>AI 인증</h2><p className="auth-status">연결됨 · 예제 계정</p><Button density="compact" className="settings-action" onClick={() => setFeedback('인증 상태 예제입니다.')}>상태 새로고침</Button></section><section><h2>새 대화의 기본값</h2><AiControls defaults info={models} selection={selection} disabled={disabled} change={setSelection} /></section><section><h2>파일 수정 권한</h2><p className="muted small">요청마다 내용을 확인한 뒤 허용 또는 거절합니다.</p></section></>}{feedback && <p role="status">{feedback}</p>}</div>}</MainPanel> : <section className="chat" aria-label="Chat"><header className="chat-heading"><div><span className="eyebrow">Chat</span><h1>{chatTitles[chatIndex]}</h1></div><span className="spacer" /><span className="run-state">{scenario === 'running' ? '응답 중' : '저장된 기록'}</span></header>
    ...
    <div className="compose-area">{approval}{scenario === 'error' && <p className="prototype-error" role="alert">연결이 끊겼습니다. 마지막으로 확인한 기록입니다. <Button onClick={() => navigate({scene:'connection', view:'main'})}>연결 확인</Button></p>}<div className="compose"><TextArea presentation="plain" aria-label="메시지" value={draft} onChange={e => setDraft(e.target.value)} disabled={disabled} rows={2} /><div className="compose-controls"><AiControls compact info={models} selection={selection} disabled={disabled || scenario === 'running'} change={setSelection} /><span className="spacer" /><IconButton shape="round" className="compose-send" label={scenario === 'running' ? '중지' : '전송'} variant="primary" disabled={disabled || !draft.trim()} onClick={() => setFeedback('시안에서는 AI 요청을 보내지 않습니다.')}><Icon name={scenario === 'running' ? 'stop' : 'arrow'} /></IconButton></div></div><p className="compose-help" role="status">{feedback || '비교용 시안 · 입력과 버튼을 체험할 수 있습니다.'}</p></div></section>}
  </WorkspaceShell>
...
function ControlSamples({ state }: { state: State }) {
  ...
  const error = state === 'error' ? '입력 내용을 다시 확인해 주세요.' : undefined;
  return <div className="control-samples"><p>제품에 적용한 실제 공통 컴포넌트입니다. 크기와 강조, 키보드 조작을 확인해 주세요.</p>
    <div className="appearance-actions"><Button variant="primary" disabled={disabled} onClick={() => setMessage('주요 조치를 눌렀습니다.')}>주요 조치</Button><Button variant="outline" disabled={disabled} onClick={() => setMessage('보조 조치를 눌렀습니다.')}>보조 조치</Button><Button disabled={disabled} onClick={() => setMessage('일반 조치를 눌렀습니다.')}>일반 조치</Button><IconButton label="조치 닫기 예제" disabled={disabled} onClick={() => setMessage('아이콘 조치를 눌렀습니다.')}><Icon name="close" /></IconButton></div>
    <Button loading={state === 'running'} disabled={state === 'disabled'} variant="outline">{state === 'running' ? '확인 중…' : '상태 확인'}</Button>
    <div className="control-fields">
    <Field label="자료 제목" description="검토할 자료의 이름을 입력하세요." error={error}>{props => <Input {...props} value={value} onChange={e => setValue(e.target.value)} disabled={disabled} />}</Field>
    <Field label="검토 방식" description="native Select의 키보드 조작을 유지합니다." error={error}>{props => <Select {...props} defaultValue="review" disabled={disabled}><option value="review">검토 후 적용</option><option value="draft">초안 저장</option></Select>}</Field>
    <Field label="저장 위치" description="이 예제의 경로는 읽기 전용입니다.">{props => <Input {...props} value="회의/제품 검토 메모.md" readOnly />}</Field>
    <Field label="검토 의견" error={error}>{props => <TextArea {...props} placeholder="의견을 입력하세요" rows={3} disabled={disabled} />}</Field>
    </div>
    <div className="control-compact"><p>Chat의 compact 선택 · 원형 전송</p><div className="chat"><div className="compose-controls"><AiControls compact info={models} selection={selection} disabled={disabled} change={setSelection} /><IconButton shape="round" className="compose-send" variant="primary" label="전송 예제" disabled={disabled}><Icon name="arrow" /></IconButton></div></div></div>
