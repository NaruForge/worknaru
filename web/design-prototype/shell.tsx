...
export function WorkspaceShell({ name, online, sidebar, children, connection, settings, theme, toggleTheme, listOpen, toggleList, navigationLabel, devStop, sidebarLabel = 'Module 목록', chat, settingsActive = false, sectionLabel = '기본 제공 Module', stacked }: {
  ...
  return <div className={`window${stacked ? ' window--stacked' : ''}`}>
    <header className="chrome">
      ...
      <Button density="compact" className="connection" onClick={connection}><span className={online ? 'dot' : 'dot offline'} />{online ? '연결됨' : '연결 끊김'}</Button>
      <IconButton density="compact" className="theme" onClick={toggleTheme} label={theme === 'dark' ? '밝은 테마' : '어두운 테마'}><Icon name={theme === 'dark' ? 'sun' : 'moon'} /></IconButton>
    </header>
    <div className="layout">
      ...
      </> : <div className="module-tools">{toggleList && <IconButton density="compact" onClick={toggleList} label={navigationLabel ?? '목록 열기 또는 접기'} aria-expanded={listOpen}><Icon name="list" /></IconButton>}<span>{sectionLabel}</span></div>}
